...
func (p S3Proxy) flushInterval(req *http.Request, res *http.Response) time.Duration {
	...
	if p.isBidirectionalStream(req, res) {
		return -1
	}
...
func (p S3Proxy) isBidirectionalStream(req *http.Request, res *http.Response) bool {
...
func (p S3Proxy) copyResponse(dst io.Writer, src io.Reader, flushInterval time.Duration) error {
	if flushInterval != 0 {
...
// of bytes written.
func (p S3Proxy) copyBuffer(dst io.Writer, src io.Reader, buf []byte) (int64, error) {
	if len(buf) == 0 {
	...
	for {
		...
		if rerr != nil && rerr != io.EOF && rerr != context.Canceled {
			...
			p.log.Error("reading from backend", zap.Error(rerr))
		}
		if nr > 0 {
